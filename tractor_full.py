...
OUTPUT_VIDEO     = "tracked_rc_car_full.mp4"   # output video file
FPS              = 30                        # frame rate
OUTPUT_FRAME_DIR = './output_frame_full'
...
APP_HIST_LEN    = 5                          # the length of history appearance                       
APP_SIM_THR     = 0.2                        # the threshold of appearance similarity
...
def init_kalman(box):
    ...
    kf.P *= 10.0
    kf.Q *= 1e-2
    kf.x[:4] = xyah_to_state(box).reshape((4, 1))
    return kf


# Tracking data structure 
class Track:
    def __init__(self, box, tid, crop_feat):
        self.box = box         # the bbox (xyxy) of object in previous frame
        self.tid = tid         # the tracting object ID
        self.kf = init_kalman(box)

        self.miss_frames = 0           # the number of frames that the object ID is missing
        self.appearance_feats = [crop_feat]  # the appearance features of current object and its history
        self.box_history = []          # the history bbox of the current object, not including the current one                 
        self.lost_age = 0

    def predict(self):
        self.kf.predict()
        self.box = state_to_xyxy(self.kf.x.flatten())

    def update(self, new_box, new_feat):
        self.box_history.append(self.box)
        self.kf.update(xyah_to_state(new_box))
        self.box = state_to_xyxy(self.kf.x.flatten())
        if len(self.box_history) > HIST_LEN:
            self.box_history.pop(0)
        
        self.appearance_feats.append(new_feat)
        if len(self.appearance_feats) > APP_HIST_LEN:
            self.appearance_feats.pop(0)

    def check_appearance(self, feat):
        sims = [cosine_similarity(f.unsqueeze(0), feat.unsqueeze(0)).item()
                for f in self.appearance_feats]
...
def crop_with_context(img, box, pad_ratio=0.3):
    h, w = img.shape[:2]
...
def init_object_trajectory(first_frame, init_center_points, model, device, apperance_model):
    """
    ...
    for center_point in init_center_points:
        ...
        crop = crop_with_context(init_frame, init_box)
        feat = extract_feature(apperance_model, crop, device)
        tracks.append(Track(init_box, next_id, feat)); next_id += 1
...
def video_tractor():
    ...
    print(f'Loading apperance model ...')
    apperance_model = load_apperance_model(device=device)
    ...
    tracks, lost_tracks = init_object_trajectory(frames[0], INIT_CENTER_PT, model, device, apperance_model)
    ...
    for fpath in tqdm(frames, desc="Tracking"):
        ...
        det_boxes, _ = detect(model, frame_tensor)
        features = [extract_feature(apperance_model, crop_with_context(frame, d), device) for d in det_boxes]
        ...
        for i, trk in enumerate(tracks):
            for j, det in enumerate(det_boxes):
                ...
                sim = trk.check_appearance(features[j])
                if sim < APP_SIM_THR: continue
                cost_mat[i, j] = W_IOU * (1 - iou) +  W_DIST * dist + W_APP*(1 - sim)
        ...
        for r, c in zip(row_ind, col_ind):
            det_box = det_boxes[c]
            if cost_mat[r, c] < (1 - IOU_THR):
                if tracks[r].check_consistency(det_box):      # IoU >= IOU_THR
                    tracks[r].update(det_box, features[c])
                    tracks[r].miss_frames = 0
                    assigned_dets.add(c)
                else:
                    tracks[r].update(tracks[r].box_history[-1], tracks[r].appearance_feats[-2])  # the most appropriate history point
                    tracks[r].miss_frames = 0
            else:
        ...
        for t in lost_tracks:
            ...
            for j, det in enumerate(det_boxes):
                ...
                sim = t.check_appearance(features[j])
                if t.check_consistency(det) and sim > APP_SIM_THR:
                    t.update(det, features[j])
                    t.miss_frames = 0
                    t.lost_age = 0
                    tracks.append(t)
                    assigned_dets.add(j)
